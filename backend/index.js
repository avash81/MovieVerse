...
if (fs.existsSync(buildPath)) {
  ...
  console.warn(
    `Warning: Directory ${buildPath} does not exist. Static files will not be served.`,
  );
}
...
try {
  aiAutomationRouter = require("./src/routes/aiAutomation");
  app.use("/api/ai", aiAutomationRouter);
  console.log("AI Automation route loaded successfully");
} catch (err) {
  console.error("Failed to load AI Automation route:", err.message, err.stack);
}

try {
  automationRouter = require("./src/routes/automation");
  app.use("/api/automation", automationRouter);
  console.log("Automation route loaded successfully");
} catch (err) {
  console.error("Failed to load Automation route:", err.message, err.stack);
}

app.use("/api/hdhub", require("./src/routes/hdhubRoutes"));
app.use("/api/seo", require("./src/routes/seoRoutes"));
app.use("/api/auto-pilot", require("./src/routes/autoPilotRoutes"));
...
app.get("*", (req, res) => {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    res.status(404).json({
      message:
        "Frontend build not found. Please run `npm run build` in the frontend directory and copy the build folder to backend/public.",
    });
  }
});
    ...
    app
      ...
      .on("error", (err) => {
        if (err.code === "EADDRINUSE") {
          console.error(
            `Port ${PORT} is already in use. Please free the port or change PORT in .env.prod.`,
          );
        } else {
