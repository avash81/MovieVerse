  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const tmdbTrailer = await fetchTmdbTrailer(
          externalId,
          movieData.title,
          releaseYear,
        );
        if (tmdbTrailer) {
          ...
          if (fallbackTrailer) {
            ...
            console.log(
              "No fallback trailer available for externalId",
              externalId,
            );
          }
        }
        ...
        if (!movieData.screenshots || movieData.screenshots.length === 0) {
          console.log(
            "Fetching screenshots from TMDb for externalId",
            externalId,
          );
          ...
          if (tmdbScreenshots) {
            ...
            console.log(
              "No screenshots found from TMDb for externalId",
              externalId,
            );
            movieData = {
              ...movieData,
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          err.message ||
          "Failed to submit review. Please check server logs or try again later.",
      );
    }
  };
  ...
  const handleReply = async (reviewId, e) => {
    ...
    try {
      console.log("Submitting reply:", {
        ...
        externalId,
        reviewId,
        data: { text: replyText, name: replyName, email: replyEmail },
      });
      ...
      setError(
        err.response?.data?.message ||
          err.message ||
          "Failed to submit reply. Please check server logs or try again later.",
      );
    } finally {
  ...
  return (
    ...
    >
      <Helmet>
        <title>
          {movie?.title
            ? `${movie.title} - MovieVerse`
            : "Movie Details - MovieVerse"}
        </title>
        <meta
          name="description"
          content={movie?.overview || "View movie details on MovieVerse"}
        />
      </Helmet>
        ...
        {movie && (
          <>
            ...
            {movie.screenshots && movie.screenshots.length > 0 ? (
                      ...
                      <div className="movie-poster">
                        <LazyLoad height={300}>
                          <img
                            src={
                              screenshot ||
                              "https://placehold.co/200x300?text=No+Screenshot"
                            }
                            alt={`${movie.title} screenshot ${index + 1}`}
                            onError={(e) =>
                              handleImageError(
                                e,
                                `${movie.title} screenshot ${index + 1}`,
                              )
                            }
                            style={{
                              width: "200px",
              ...
              <p
                style={{ fontSize: "1rem", textAlign: "center", color: "#ccc" }}
              >
            ...
            <div className="download-links" style={{ margin: "20px 0" }}>
              ...
              {movie.directLink ? (
                ...
                <p style={{ fontSize: "1rem", color: "#ccc" }}>
                  No direct watch links available.
                </p>
              )}
            ...
            >
              {watchlist.some(
                (m) =>
                  m.externalId === movie.externalId &&
                  m.source === movie.source,
              ) ? (
                <button
                  className="cta-button"
                  onClick={() =>
            ...
            >
              ...
              <p>
                <strong>Budget:</strong> {movie.budget || "N/A"}
              </p>
              <p>
                <strong>Revenue:</strong> {movie.revenue || "N/A"}
              </p>
              <p>
                  ...
                  >
                    ...
                    <div className="comment-content">
                      ...
                      {!replyingTo || replyingTo !== review._id ? (
                        ...
                        <form
                          onSubmit={(e) => handleReply(review._id, e)}
