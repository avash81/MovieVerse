...
const MovieCard = ({
  ...
  return (
    <div
      key={`${categoryId}-${movie.source}-${movie.externalId}-${index}`}
      ...
      >
        ...
        <div className="movie-overlay">
          ...
          <Link
            ...
            onClick={() =>
              trackClick("view_details", movie.externalId, movie.title)
            }
          >
            <button
          ...
          {movie.directLink && (
            <a
              ...
              className="watch-now"
              aria-label={`Watch ${movie.title} on Internet Archive`}
            >
              Watch Now
            </a>
          )}
          {watchlist.some(
            (m) =>
              m.externalId === movie.externalId && m.source === movie.source,
          ) ? (
            <button
              className="cta-button"
              onClick={(e) => {
...
function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setNotices(
          Array.isArray(noticesResponse.data) ? noticesResponse.data : [],
        );
      } catch (err) {
      ...
      const categoryPromises = categories.map(async (category) => {
        try {
          ...
          if (error && error.includes("404")) {
            console.warn(
              `Skipping category ${category.id} due to 404 Not Found`,
            );
          ...
          setError((prev) =>
            prev
              ? `${prev} Error loading ${category.name}: ${err.message || "Failed to load"}.`
              : `Error loading ${category.name}: ${err.message || "Failed to load"}.`,
          );
          return { id: category.id, name: category.name, movies: [] };
        }
      });
      ...
      if (
        newMoviesByCategory["trending"] &&
        newMoviesByCategory["trending"].length > 0
      ) {
          ...
          movies.map(async (movie) => {
            try {
              ...
              return {
                ...
                count: Array.isArray(reviewsResponse.data)
                  ? reviewsResponse.data.length
                  : 0,
              };
            } catch (err) {
              console.error(
                `Error fetching reviews for ${movie.source}/${movie.externalId}:`,
                err,
              );
  ...
  const handleAddToWatchlist = (movie) => {
    ...
    const newWatchlist = [
      ...watchlist,
      { ...movie, source: movie.source, externalId: movie.externalId },
    ];
  ...
  const getRecommendations = () => {
    ...
    return (
      moviesByCategory["trending"]
        ?.filter((movie) => movie.genre_ids?.includes(topGenre))
        ?.slice(0, 5) || []
    );
  };
  ...
  const memoizedCategories = useMemo(() => {
    if (
      ...
      searchQuery.toLowerCase().includes("web series")
    ) {
      return [
    ...
    <div className="home fade-in">
      ...
      {quickViewMovie && (
        <div className="quick-view-modal">
          <button onClick={closeQuickView} aria-label="Close quick view">
            ×
          </button>
          <h2>{quickViewMovie.title}</h2>
          <p>
            <strong>Overview:</strong>{" "}
            {quickViewMovie.overview || "No overview available."}
          </p>
          <p>
            <strong>Cast:</strong> {quickViewMovie.cast?.join(", ") || "N/A"}
          </p>
          <p>
            <strong>Budget:</strong>{" "}
            {quickViewMovie.budget
              ? `$${quickViewMovie.budget.toLocaleString()}`
              : "N/A"}
          </p>
          <p>
            <strong>Release Date:</strong>{" "}
            {quickViewMovie.release_date || "N/A"}
          </p>
          <p>
            <strong>Genres:</strong>{" "}
            {quickViewMovie.genres?.join(", ") || "N/A"}
          </p>
          <p>
            <strong>IMDb Rating:</strong> {quickViewMovie.imdbRating || "N/A"}
          </p>
        </div>
      )}
          ...
          <nav className="nav">
            {categories.map((category) => (
              <span
                ...
                tabIndex={0}
                onKeyPress={(e) =>
                  e.key === "Enter" && scrollToCategory(category.id)
                }
            ...
            <span
              ...
              tabIndex={0}
              onKeyPress={(e) =>
                e.key === "Enter" && scrollToCategory("watchlist")
              }
            ...
            <Link
              to="/ai-automation"
              className="nav-link"
              aria-label="Navigate to AI Automation"
            >
              🤖 AI Tools
            </Link>
            <Link
              to="/task-automation"
              className="nav-link"
              aria-label="Navigate to Task Automation"
            >
              🚀 Auto Tasks
            </Link>
            <Link
        ...
        <div className="hero-section">
          <img
            src={
              ...
              "https://placehold.co/3840x2160?text=4K+Poster"
            }
            alt={featuredMovie.title}
            onError={(e) => {
              e.target.src = "https://placehold.co/3840x2160?text=4K+Poster";
            }}
          />
          <div className="hero-overlay">
            <div className="hero-content">
              ...
              <p>
                IMDb Rating: {featuredMovie.imdbRating || "N/A"} | Total Number
                of Reviews: {reviewCounts[featuredMovie.externalId] || 0}
              </p>
              <div className="hero-buttons">
                ...
                {watchlist.some(
                  (m) =>
                    m.externalId === featuredMovie.externalId &&
                    m.source === featuredMovie.source,
                ) ? (
                  <button
                    className="cta-button hero-button"
                    onClick={() =>
      ...
      {watchlist.length > 0 && (
        <div className="category-section" id="watchlist">
          ...
          <div className="movie-grid horizontal-scroll">
            {watchlist
              .filter((movie) => movie.source && movie.externalId)
              .map((movie, index) => (
                <MovieCard
